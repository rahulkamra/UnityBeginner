...
public static class SurfaceNoise
{
    ...
    public static SurfaceNoiseMethod[] ValueMethods = { SurfaceNoise.Value1D, SurfaceNoise.Value2D, SurfaceNoise.Value3D };
    public static SurfaceNoiseMethod[] PerlinMethods = { SurfaceNoise.Perlin1D, SurfaceNoise.Perlin2D, SurfaceNoise.Perlin3D };

    public static SurfaceNoiseMethod[][] NoiseMethods = {
        ValueMethods,
        PerlinMethods
    };
    ...
    private const int gradientMaskID = 1;
    ...
    public static float Perlin1D(Vector3 point, float frequency)
    {
        ...
        point *= frequency;
        int intX0 = Mathf.FloorToInt(point.x);
        float t0 = point.x - intX0;
        float t1 = t0 - 1f;

        intX0 &= hashMask;
        int intX1 = intX0 + 1;

        float g0 = gradients1D[hash[intX0] & gradientMaskID];
        float g1 = gradients1D[hash[intX1] & gradientMaskID];
